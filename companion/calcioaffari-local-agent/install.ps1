...
function Test-WordPressConnection {
    ...
    try {
        ...
        if ([version][string]$health.version -lt [version]"0.8.4") {
            throw (New-CalcioAffariException "CA_PLUGIN_OUTDATED" "Aggiorna CalcioAffari News Engine alla versione 0.8.4 o successiva.")
        }
        return $health
    }
    catch {
...
function Install-Agent {
    ...
    foreach ($file in @(
        ...
        "Apri-CalcioAffari.cmd", "Disinstalla-CalcioAffari.cmd", "version.json", "README.md", "AUDIT-1.0.6.md"
    )) {
