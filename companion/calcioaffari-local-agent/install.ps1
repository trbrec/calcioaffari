...
function Test-WordPressConnection {
    ...
    try {
        ...
        if ([version][string]$health.version -lt [version]"0.8.3") {
            throw (New-CalcioAffariException "CA_PLUGIN_OUTDATED" "Aggiorna CalcioAffari News Engine alla versione 0.8.3 o successiva.")
        }
        return $health
    }
    catch {
